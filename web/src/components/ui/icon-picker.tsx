...
const useIconsData = () => {
  ...
  useEffect(() => {
    ...
    const loadIcons = async () => {
      ...
      if (isMounted) {
        setIcons(
          iconsData.filter((icon: IconData) => icon.name in dynamicIconImports)
        );
        setIsLoading(false);
      }
    };
...
const DEBOUNCED_SEARCH_DELAY = 100;
...
>(
  (
    ...
    const [search, setSearch] = useDebounceValue("", DEBOUNCED_SEARCH_DELAY);
        ...
        >
          {virtualizer.getVirtualItems().map((virtualItem: VirtualItem) => {
            ...
            if (item.type === "category") {
              return (
                <div
                  className="top-0 z-10 bg-zinc-900"
                  key={virtualItem.key}
                  style={itemStyle}
                >
                  <h3 className="mb-2 font-medium text-sm capitalize">
